	...
	<body onload="init()">
		...
		<script type="text/javascript">
			...
			function startTime() {
				...
				if (H<5){
					txt='Have a Good Night Tori-chan:)';
					if(txt != txt2 && !starting)
						document.getElementById("quote").innerHTML = txt;
					txt2=txt;
				}
				else if(H<9){
					txt='Ohayou Tori-chan:)';
					if(txt != txt2 && !starting)
						document.getElementById("quote").innerHTML = txt;
					txt2=txt;
				}
				else if(H<12){
					txt='Selamat beraktifitas Tori-chan,Udh makan?';
					if(txt != txt2 && !starting)
						document.getElementById("quote").innerHTML = txt;
					txt2=txt;
				}
			...
			const quotes = [
			  ...
			  {
			    "quote": "Laughing as we held each other hands,i felt that it would go on forever",
			    "source": "Miku"
			  },
			]
